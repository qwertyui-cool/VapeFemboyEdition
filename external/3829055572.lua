...
runcode(function()
	local ToolSpammer = {["Enabled"] = false}
	ToolSpammer = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		["Name"] = "ToolSpammer",
		["Function"] = function(callback)
			if callback then
				local hand = lplr.Backpack.Swuvle or lplr.Character.Swuvle
				local hand2 = lplr.Backpack["Sniper 2"] or lplr.Backpack.Sniper or lplr.Character.Sniper or lplr.Character["Sniper 2"]
				RunLoops:BindToStepped("ToolSpammer",1,function()
					lplr.Character.Humanoid:EquipTool(hand)
					lplr.Character.Humanoid:EquipTool(hand2)
					hand.Parent = lplr.Backpack
					hand2.Parent = lplr.Backpack
				end)
			else
				RunLoops:UnbindFromStepped("ToolSpammer")
			end
		end,
		["HoverText"] = "This is a base module."
	})
